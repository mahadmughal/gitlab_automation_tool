...
class GitLabPipelineAutomator {
    ...
    async connectToExistingChrome() {
        try {
            ...
            console.log("Please start Chrome with: /Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome --remote-debugging-port=9222");
            return false;
        }
    }
    ...
    async selectBranch(branchName = "production") {
        ...
        try {
            await this.driver.get('https://devops.housing.sa:8083/ejar3/devs/ejar3-run-script-tool/-/pipelines/new');
            await this.reloadPage();

            await this.driver.wait(until.elementLocated(By.tagName('fieldset')), 10000);

            console.log("Navigating to GitLab pipeline page...");
            await this.driver.get('https://devops.housing.sa:8083/ejar3/devs/ejar3-run-script-tool/-/pipelines/new');

            const refSelectorDiv = await this.driver.wait(
                until.elementLocated(By.className('ref-selector')), 10000
            );

            const dropdownButton = await refSelectorDiv.findElement(By.id('dropdown-toggle-btn-34'));
            await dropdownButton.click();

            const baseDropdownDiv = await this.driver.wait(
                until.elementLocated(By.id('base-dropdown-36')), 10000
            );
    ...
    async processCiVariables(ticketDescription, script, ejarService) {
        try {
            ...
            const ciVariableRowContainers = await this.driver.wait(
                until.elementsLocated(By.css('div[data-testid="ci-variable-row-container"]')), 10000
            );

            if (ejarService.toLowerCase().includes("sec")) {
                console.log("🔍 'sec' keyword detected in ejar_service - extracting ticket description from script...");

                const extractedDescription = this.fetchTicketDescriptionFromScript(script);

                if (extractedDescription) {
                    ticketDescription = extractedDescription;
                    console.log(`✓ Updated ticket description to: '${ticketDescription}'`);
                } else {
                    console.log("⚠️ Could not extract ticket description, using original ticket description");
                }
            }

            console.log("Setting ticket description in first container...");
            const firstContainer = ciVariableRowContainers[0];
            const textarea = await firstContainer.findElement(By.css('[data-testid="pipeline-form-ci-variable-value-field"]'));
            ...
            const dropdownBtn = await secondContainer.findElement(By.css('[data-testid="pipeline-form-ci-variable-value-dropdown"]'));
            await dropdownBtn.click();
            await this.sleep(1000);

            const dropdownDiv = await this.driver.wait(
                until.elementLocated(By.id('base-dropdown-59')), 10000
            );
            ...
            for (const option of serviceOptions) {
                ...
                if (optionText && optionText.toLowerCase().includes(ejarService.toLowerCase())) {
                    await option.click();
                    serviceSelected = true;
                    console.log(`✓ Selected service: '${optionText.replace('listbox-item-', '')}'`);
                    break;
                }
            }

            if (!serviceSelected) {
                console.log(`✗ Service '${ejarService}' not found, using default (5th option)`);
            ...
            const scriptTextarea = await thirdContainer.findElement(By.css('[data-testid="pipeline-form-ci-variable-value-field"]'));
            ...
            await this.driver.executeScript("window.scrollTo(0, document.body.scrollHeight - 500);");
            await this.sleep(1000);

            console.log("Clicking the Run Pipeline button...");
            const runPipelineButton = await this.driver.wait(
                until.elementLocated(By.css('[data-testid="run-pipeline-button"]')), 10000
            );
    ...
    async waitForPipelinePage() {
        try {
            ...
            const pipelinePageLoaded = async (driver) => {
                ...
                return currentUrl.startsWith('https://devops.housing.sa:8083/ejar3/devs/ejar3-run-script-tool/-/pipelines/');
            };
            ...
            try {
                ...
                console.log(`⚠️ Timeout waiting for pipeline page, continuing anyway: ${error}`);
                return true;
            }
        } catch (error) {
        ...
        try {
            ...
            try {
                ciBadgeRequestProdDiv = await this.driver.wait(
                    until.elementLocated(By.id('ci-badge-request_prod')), 20000
                );
                ...
                try {
                    ciBadgeRequestProdDiv = await this.driver.wait(
                        until.elementLocated(By.css('[id*="ci-badge-request"]')), 10000
                    );
            ...
            while (attempt < maxAttempts) {
                try {
                    ...
                    const ciIcon = await ciBadgeRequestProdDiv.findElement(By.css('[data-testid="ci-icon"]'));
                    const iconClass = await ciIcon.getAttribute('class');

                    console.log(`Attempt ${attempt + 1}: Request stage status - ${iconClass}`);

                    if (iconClass.includes('ci-icon-variant-success')) {
                        console.log("✓ Request stage completed successfully!");
                        return true;
                    } else if (iconClass.includes('ci-icon-variant-failed') || iconClass.includes('ci-icon-variant-error')) {
                        console.log("✗ Request stage failed!");
                        return false;
                    } else {
                        console.log("⏳ Request stage still in progress so try reload page...");
                        await this.reloadPage();
                    }
                } catch (error) {
            ...
            while (attempt < maxAttempts) {
                try {
                    const ciBadgeApproveProdDiv = await this.driver.wait(
                        until.elementLocated(By.id('ci-badge-approve_prod')), 20000
                    );
                    const ciIcon = await ciBadgeApproveProdDiv.findElement(By.css('[data-testid="ci-icon"]'));
                    const iconClass = await ciIcon.getAttribute('class');
                    const neutralCiIcon = iconClass.includes('badge-neutral');

                    if (neutralCiIcon) {
                        const approveButton = await ciBadgeApproveProdDiv.findElement(By.css('[data-testid="ci-action-button"]'));
                        await this.driver.executeScript("arguments[0].scrollIntoView(true);", approveButton);
                        await this.sleep(2000);
                        await approveButton.click();
                        await this.sleep(2000);

                        const updatedDiv = await this.driver.findElement(By.id('ci-badge-approve_prod'));
                        const updatedIcon = await updatedDiv.findElement(By.css('[data-testid="ci-icon"]'));
                        ...
                        while (approveAttempt < maxAttempts) {
                            const approveDiv = await this.driver.findElement(By.id('ci-badge-approve_prod'));
                            const approveIcon = await approveDiv.findElement(By.css('[data-testid="ci-icon"]'));
                            ...
                            if (successCiIcon) {
                                ...
                                console.log("✗ Approve stage still in progress so try reload page...");
                                await this.reloadPage();
                                console.log(`⏳ Checking... (attempt ${approveAttempt}/${maxAttempts})`);
                                await this.sleep(5000);
                            }
                            approveAttempt++;
                        }
    ...
    async runPipelineStage() {
        try {
            ...
            const ciBadgeRunscriptProdDiv = await this.driver.wait(
                until.elementLocated(By.id('ci-badge-runscript_prod')), 10000
            );
    ...
    async runAutomation(branchName = "production", ticketDescription = "", script = "", ejarService = "") {
        try {
            if (!ticketDescription || !script || !ejarService) {
                console.log("Error: All parameters (ticket_description, script, ejar_service) are required");
                return false;
            }
            ...
            if (!await this.processCiVariables(ticketDescription, scriptContent, ejarService)) {
